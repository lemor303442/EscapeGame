...
public class SpriteCommandHandler : ScenarioCommandHandler
{
    public class Options : CommandOptions
    {
        ...
        public static Options Create(Scenario scenario)
        {
            ...
            if (image == null)
            {
                Debug.LogWarningFormat("Sprite Error: [{0}] not found", scenario.Arg2);
            }
            return new Options(layer, image);
        }
    }

    public SpriteCommandHandler(ScenarioManager scenarioManager) : base(scenarioManager)
    {
    }
